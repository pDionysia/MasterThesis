...
from tsai.all import InceptionTime, Learner, TSDataLoaders, accuracy, get_splits
...
X100, y100 = load_folder(
    r"C:\Dev\MasterThesis\data\SRRs_with_100ppb_Acetamiprid", label=3
)
X1000, y1000 = load_folder(
    r"C:\Dev\MasterThesis\data\SRRs_with_1000ppb_Acetamiprid", label=4
)
...
dls = TSDataLoaders.from_numpy(X_tsai, y_tsai, splits=splits, bs=32, num_workers=0)
...
def train_model(model_cls, model_name, **kwargs):
    ...
    dls = TSDataLoaders.from_numpy(X_model, y_tsai, splits=splits, bs=32, num_workers=0)
    ...
    return {"name": model_name, "loss": float(valid_loss), "acc": float(valid_acc)}
...
results.append(train_model(OmniScaleCNN, "OmniScaleCNN", seq_len=series_length))
